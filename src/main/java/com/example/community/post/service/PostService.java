...
public class PostService {
    ...
    // 게시글 저장, 조회, 수정, 삭제를 담당하는 Repository
    ...
    public PostResponse createPost(Long memberId, PostCreateRequest request) {
        ...
        // 그래서 post.setId(++sequence)를 직접 할 필요가 없다.
        Post savedPost = postRepository.save(post);

        // 4. 저장된 Post 엔티티를 응답 DTO로 변환해서 반환
        //
    ...
    public List<PostResponse> getPosts() {
        ...
        // 1. postRepository.findAll()로 DB에 저장된 모든 게시글을 조회
        // JpaRepository가 기본 제공하는 메서드라서 직접 SQL을 작성하지 않아도 된다.
        return postRepository.findAll()

                // 2. 조회 결과는 List<Post> 형태
    ...
    public PostResponse updatePost(Long postId, Long memberId, PostUpdateRequest request) {
        ...
        // 1. 수정하려는 게시글을 DB에서 조회
        // 게시글이 없으면 수정할 수 없으므로 예외를 발생시킨다.
        Post post = postRepository.findById(postId)
                .orElseThrow(() -> new IllegalArgumentException("게시글을 찾을 수 없습니다."));

        // 2. 로그인한 사용자와 게시글 작성자가 같은지 확인
        // post.getMemberId()는 Post 안에 있는 Member 객체에서 memberId를 꺼내는 메서드
        // --> 이 게시글을 작성한 회원의 ID를 의미
        //
        ...
        // Post 엔티티 안에 update() 메서드를 만들어서 수정
        // --> 게시글 수정 규칙을 Post 엔티티 안에 모아둘 수 있기 때문이다.
        post.update(
                request.getTitle(),
                request.getContent(),
                null
